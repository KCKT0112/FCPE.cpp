...
// Independent scalar oracle for the Vulkan F32 specialization. Odd dimensions
    ...
    try {
        ...
        for (const auto & s : shapes) {
            ...
            for (int z=0; z<batches; ++z) for (int j=0; j<n; ++j) for (int i=0; i<m; ++i) {
                double reference = 0;
                for (int q=0; q<k; ++q) reference += double(av[z*m*k + (s[4] ? q*m+i : i*k+q)]) * bv[z*n*k+j*k+q];
                const float got = output[z*m*n+j*m+i];
                const float error = static_cast<float>(std::abs(got-reference));
                if (!std::isfinite(got) || error > 1e-4 + 5e-6*std::abs(reference))
                    throw std::runtime_error("Matmul mismatch at M=" + std::to_string(m) + ", N=" + std::to_string(n) + ", K=" + std::to_string(k));
